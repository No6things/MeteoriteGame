...
bool CTextures::LoadTexture(char *mFileName, UINT &mTexture)
{
	...
	//mFileName just contain the name of the texture
	strcat(tempstring2,"Assets/Models/Firebird/");
	strcat(tempstring2,mFileName);
	cout<<tempstring2<<endl;
	if (!FileExist(tempstring2))							//check to see if the file exists
	{	
	...
	{	
		mImage = LoadJPG(tempstring2);	
	}	
	else if((!strcmp(tempstring, ".bmp"))||(!strcmp(tempstring, ".BMP")))
	{	
		mImage = LoadBMP(tempstring2);	
	}
	else
	{	return false;		}
